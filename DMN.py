	...
	def train_batch(self, sess, batch, sum_writer):
		for (type, gradient_descent, summary_op_for_that_type) in [
			...
			if len(feed_dict[self.type]) > 0:
				_, glo_step, summary_all, specialized_summary = sess.run(
					[gradient_descent, self.global_step, self.merged_summary_op, summary_op_for_that_type],
					feed_dict=feed_dict)
				sum_writer.add_summary(summary_all, global_step=glo_step)
				sum_writer.add_summary(specialized_summary, global_step=glo_step)

	def test_batch(self, sess, batch):
		ret_list = []
		for (type, predicts, accuracy) in [('b', self.predicts_b, self.accuracy_b),
