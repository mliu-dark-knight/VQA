...
class DMN(Base):
	...
	def build_input(self):
		...
		facts = fully_connected(input, self.params.hidden_dim, 'ImageEmbedding', activation=None, bn=True, training=self.training)
	...
	def build_type(self, question):
		with tf.variable_scope('Question_Type'):
			return fully_connected(question, self.params.num_ques_type, 'Type', activation=None)
	...
	def build_logits(self, memory, vocab_size, prefix, type):
		with tf.variable_scope('Answer'):
			return fully_connected(memory, vocab_size, prefix, activation=None, type=type)
