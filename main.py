...
# training options
flags.DEFINE_integer('batch_size', 8, 'Batch size during training and testing')
...
flags.DEFINE_integer('num_range', 31, 'Range of numerical answer')
flags.DEFINE_integer('hidden_dim', 850, 'Size of hidden units')
...
flags.DEFINE_float('epsilon', 0.05, 'Annealing parameter for attention softmax')
flags.DEFINE_integer('max_ques_size', 15, 'Max length of question')
flags.DEFINE_float('lambda_r', 0.001, 'Regularization weight')
...
flags.DEFINE_integer('rnn_layer', 4, 'Number of layers in question encoder')
...
def main(_):
	...
	tboard_proc = subprocess.Popen(shlex.split(
		'/home/victor/anaconda3/bin/tensorboard --logdir=/home/victor/ECE547/' + FLAGS.save_dir + 'log'))

	os.makedirs(FLAGS.save_dir)

	word2vec = WordTable.load_word2vec()
	FLAGS.vocab_size = word2vec.vocab_size
	dataset = DataSet(word2vec=word2vec, params=FLAGS, type='train', q_max=50, num_threads=7)
